...
<#
.SYNOPSIS
## For CI use only.  Unzips zip file created by preceding job in pipeline.
#>
function Restore-Workspace([switch] $CI){
    if(!$CI){
        if(!(Check-IsGitRepo)) {
        ...
        $zipFile = (ls -Include *.zip -Recurse).Name
        if(!$zipFile) {
...
#>
    ...
    #throw exception if no sandcastle_docs in yml
    if(!$yml.sandcastle_docs){
        Write-Output "No Sandcastle Docs in YML"
    }else{
    ...
    if(!(Is-Master))
    {
           Register-WIPNugetFeed -feedName $env:GIT_BRANCH 
    }
